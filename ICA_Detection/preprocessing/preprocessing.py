...
def process_images(json_path: str, out_dir: str, steps_order: List[str]) -> None:
    ...
    for uid, entry in tqdm(iterable=dataset.items(), desc="Processing JSON entries ...", colour="green", total=len(dataset.items())):
        ...
        if "format_standarization" in entry.get("preprocessing_plan", {}) and "format_standarization" in steps_order:
            desired_format = entry["preprocessing_plan"]["format_standarization"].get("desired_format").lower()
        ...
        if "dtype_standarization" in entry.get("preprocessing_plan", {}) and "dtype_standarization" in steps_order:
        ...
        if "resolution_standarization" in entry.get("preprocessing_plan", {}) and "resolution_standarization" in steps_order:
            ...
            # Re-normalize bounding boxes: each bbox was normalized using original dimensions.
            annotations = entry.get("annotations", {})
            for key, bbox in annotations.items():
                if key == "name":
                    continue
                # Original normalized values based on orig_width and orig_height.
                old_x_center = bbox.get("x_center")
                old_y_center = bbox.get("y_center")
                old_width = bbox.get("width")
                old_height = bbox.get("height")
                # Compute pixel values from original dimensions.
                pixel_x_center = old_x_center * orig_width
                pixel_y_center = old_y_center * orig_height
                pixel_width = old_width * orig_width
                pixel_height = old_height * orig_height
                # Re-normalize using desired dimensions.
                new_x_center = pixel_x_center / desired_X
                new_y_center = pixel_y_center / desired_Y
                new_width = pixel_width / desired_X
                new_height = pixel_height / desired_Y
                # Update bbox.
                bbox["x_center"] = new_x_center
                bbox["y_center"] = new_y_center
                bbox["width"] = new_width
                bbox["height"] = new_height
            # End resolution step.
        
        # --- Step 4: Filtering Smoothing Equalization ---
        if "filtering_smoothing_equalization" in entry.get("preprocessing_plan", {}) and "filtering_smoothing_equalization" in steps_order:
