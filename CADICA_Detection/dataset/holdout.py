...
def run_saveSplit(df: pd.DataFrame, output_path: str, split_name: str) -> None:
    ...
    split_df["Frame_path"] = split_df.progress_apply(
        lambda row: (
            row["SelectedFramesLesionVideo"]
            if row["Lesion"]
            else row["SelectedFramesNonLesionVideo"]
        ),
        axis=1,
    )
    ...
    split_df["LesionLabel"] = split_df["LesionLabel"].apply(
        lambda x: x if x else ["nolesion"]
    )
