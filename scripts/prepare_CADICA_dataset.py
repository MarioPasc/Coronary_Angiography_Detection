...
logging.basicConfig(
    filename="./logs/dataset_generation.log",
    level=logging.INFO,
...
def load_config(yaml_path: str) -> dict:
    ...
    try:
        ...
        logging.error(
            "Error loading config.yaml file; Are you executing the script from the root folder? If so, check this .py and change CONFIG_PATH."
        )
...
def download_and_prepare_dataset():
    try:
        ...
        # Download dataset
        DatasetTools.downloadCADICA(
            destination_folder=destination_folder,
            url=CONFIG["DATASET_URL"],
            filename="CADICA_Dataset.zip",
        )

        # Rename extracted folder
        original_folder = os.path.join(
            destination_folder, "CADICA a new dataset for coronary artery disease"
        )
        ...
        if os.path.exists(original_folder):
            ...
            logging.warning(
                "Original dataset folder not found. Skipping renaming step."
            )
...
def generate_dataset_analysis():
    try:
        ...
        destination_folder = os.path.join(
            CONFIG["DESTINATION_FOLDER"], CONFIG["DS_NAME"]
        )
        selected_videos_path = os.path.join(
            destination_folder, "CADICA", "selectedVideos"
        )
        ...
        for patient in tqdm(
            patients_folders, desc="Processing patients...", colour="green"
        ):
            ...
            non_lesion_videos = DatasetTools.getFilePaths(
                patient_path, "nonlesionVideos.txt"
            )

            for video in lesion_videos + non_lesion_videos:
                video_path = os.path.join(patient_path, video)
                selected_frames = DatasetTools.getSelectedFrames(
                    video_path, patient, video
                )
                groundtruth_map = DatasetTools.getGroundtruthFiles(
                    video_path, lesion_videos
                )

                for frame in selected_frames:
                    frame_id = os.path.basename(frame).split("_")[-1].split(".")[0]
                    groundtruth_file = groundtruth_map.get(
                        f"{patient}_{video}_{frame_id}", ""
                    )
                    has_lesion = groundtruth_file != ""
                    lesion_labels_list = (
                        DatasetTools.extractLesionLabel(groundtruth_file)
                        if has_lesion
                        else []
                    )
...
def apply_undersampling():
    try:
        logging.info(
            "Step 4: Applying undersampling to overrepresented lesion classes."
        )
        ...
        post_undersampling_df = DatasetTools.undersampling(
            holdout_folder,
        ...
        logging.info(
            f"Step 4 complete: Undersampling applied. Modified content saved in {holdout_folder}."
        )
    except Exception as e:
...
def apply_data_augmentation():
    try:
        ...
        # We
        undersampled_datasets = os.path.join(
            CONFIG["OUTPUT_PATH"], "CADICA_Undersampled_Images"
        )
        augmentation_folder = os.path.join(
            CONFIG["OUTPUT_PATH"], "CADICA_Augmented_Images"
        )
        ...
        train_aug_counts_csv = os.path.join(
            augmentation_folder, "train_augmentation_counts.csv"
        )
        val_aug_counts_csv = os.path.join(
            augmentation_folder, "val_augmentation_counts.csv"
        )
        ...
        logging.info(
            f"Step 5 complete: Data augmentation completed. Results saved in {augmentation_folder}."
        )
    except Exception as e:
...
def format_for_yolo():
    try:
        ...
        undersampling_folder = os.path.join(
            CONFIG["OUTPUT_PATH"], "CADICA_Undersampled_Images"
        )

        train_path = os.path.join(
            augmentation_path, "processed_train.csv"
        )  # Augmented train data
        val_path = os.path.join(
            augmentation_path, "processed_val.csv"
        )  # Augmented val data
        test_path = os.path.join(
            undersampling_folder, "processed_test.csv"
        )  # Undersampled test split. This test.csv is the original since we did not remove any samples from it

        output_folder = os.path.join(
            CONFIG["DESTINATION_FOLDER"], CONFIG["YOLO_DATASET_FOLDER_NAME"]
        )
